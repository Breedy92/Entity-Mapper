...
import { EntityNode, EntityType } from '../types';
...
export const EntityCard: React.FC<EntityCardProps> = ({ 
  ...
  return (
    <div
      className={`absolute w-64 p-6 rounded-[2rem] border-2 cursor-move select-none transition-all duration-500 ease-out shadow-lg ${colors} ${opacityClass} ${isSelected ? 'ring-[6px] ring-indigo-500/10 border-indigo-500 shadow-2xl z-[100]' : 'z-40'} ${isConnectingSource ? 'border-dashed border-indigo-400 animate-pulse bg-indigo-50' : ''}`}
    ...
    >
      <div className="flex items-center gap-4 mb-4 relative z-10">
        <div className={`p-3 rounded-2xl ${isSelected ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-400'} transition-all`}>
          {getIcon(node.type)}
        </div>
      ...
      <div className={`transition-all duration-300 overflow-hidden relative z-10 ${isExpanded ? 'max-h-96' : 'max-h-12'}`}>
      ...
      {node.description && node.description.length > 50 && (
        ...
        >
          {isExpanded ? 'Collapse Details ▲' : 'Read Full Notes ▼'}
        </button>
      )}
    </div>
  );
